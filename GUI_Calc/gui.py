...
class calculator:
    ...
    def square(self):
        ...
        try:
            ...
            self.e.delete(0,END)
            self.e.replace(0,self.sqval)

    def clearall(self):
            self.e.delete(0,END)

    def clear1(self):
        sef.txt = self.e.get()[:,-1]
        self.e.delete(0,END)
    ...
    def __init__(self,master):
        master.title('GREATEST CALCULATOR')
        master.geometry()
        self.e = Entry(master)
        ...
        Button(master,text="sqrt",width=3,command=lambda:self.squareroot()).grid(row=3, column=4)
        Button(master,text="xe2",width=3,command=lambda:self.square()).grid(row=3, column=5)
